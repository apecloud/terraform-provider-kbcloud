# Redis Multi-Mode Configuration Guide

## Overview

Redis supports multiple deployment modes. The refactored configuration uses a **single flexible cluster definition** that can be configured for different modes through variables.
...
### 1. Standalone Mode
Single-node Redis instance (for development/testing).
...
class_code = "redis.cluster.redis-cluster.2c1g.general"
...
sentinel_replicas = 3
sentinel_class_code = "redis.sentinel.redis-sentinel.0.5c0.5g.general"
```

**Total components:** 
- 1 Redis component (2 replicas)
...
```hcl
cluster_name     = "redis-cluster-prod"
mode             = "cluster"
comp_num         = 6        # 6 shards
replicas         = 2        # 2 replicas per shard
network_mode     = "HeadlessService"
storage_size_gb  = 100
class_code       = "redis.cluster.redis-cluster.4c2g.performance"
component_name   = "redis-cluster"
param_tpl_name   = "redis-cluster-default-parameter-template"
```
...
```hcl
cluster_name     = "redis-sentinel-prod"
mode             = "sentinel"
replicas         = 2
network_mode     = "HostNetwork"
extra_sentinel   = "{}"

# Primary Redis component
...
- `component_name`: Should be "redis-cluster"
...
**Best for:** Complex HA requirements, existing Redis clients using Sentinel
...
```bash
./run.sh -t 4 \
    -cc "redis.replication.redis.4c4g.performance" \
    -s 100
```
...
kubectl exec -it <primary-pod> -n <namespace> -- redis-cli info replication
```

**Cluster:**
```bash
# Check cluster info
kubectl exec -it <pod> -n <namespace> -- redis-cli cluster info
kubectl exec -it <pod> -n <namespace> -- redis-cli cluster nodes
```

**Sentinel:**
```bash
# Check sentinel status
kubectl exec -it <sentinel-pod> -n <namespace> -- redis-cli -p 26379 info sentinel
```
...
3. Check if `component_name = "redis-cluster"`
...
```bash
kubectl logs -l app.kubernetes.io/component=redis-sentinel -n kubeblocks-cloud-ns
```
...
class_code = "redis.cluster.redis-cluster.1c1g.general"  # Smaller instances
```
...
2. Migrate data using Redis migration tools
...
2. Use Redis Cluster migration tools
3. Test thoroughly
...
- [KubeBlocks Redis Guide](https://kubeblocks.io/docs)
