# MySQL Terraform Test Script Guide

## 📖 Overview

The `run.sh` script provides a command-line interface for managing MySQL clusters on KubeBlocks Cloud using Terraform. It simplifies cluster operations by allowing you to configure and execute Terraform commands through simple command-line parameters.
...
|-----------|-------|------|---------|-------------|
| Cluster Name | `-cn` | `--cluster-name` | `my-mysql` | Unique cluster identifier |
| Engine | `-e` | `--engine` | `mysql` | Database engine type |
...
| Class Code | `-cc` | `--class-code` | `mysql.replication.mysql.1c2g.general` | Instance specification |
...
./run.sh -t 1 \
    ...
    -r 1 \
    -s 10 \
    -cc "mysql.replication.mysql.1c2g.general"
```

**What this does:**
- Creates a single-replica MySQL cluster
- 10GB storage
...
**What this does:**
- Updates MySQL configuration parameters
...
./run.sh -t 1 -cn "test-small" -r 1 -s 10 -cc "mysql.replication.mysql.1c2g.general"
# ... run tests ...
./run.sh -t 2

# Test medium configuration
./run.sh -t 1 -cn "test-medium" -r 2 -s 50 -cc "mysql.replication.mysql.2c4g.general"
# ... run tests ...
./run.sh -t 2
...
echo "Deploying MySQL cluster: $CLUSTER_NAME"
...
alias mysql-create='./run.sh -t 1 -cn "my-db" -r 2 -s 50'
alias mysql-scale='./run.sh -t 4 -cc "mysql.replication.mysql.2c4g.general"'
alias mysql-destroy='./run.sh -t 2'
```
...
- [MySQL Example README](README.md)
