...
DEFAULT_DISPLAY_NAME="My PostgreSQL Cluster"
...
Options:
    ...
    -bm, --backup-method                    Backup method (wal-g-archive)
    ...
    -cfn, --config-file-name                Configuration file name for reconfigure (e.g., postgresql.conf)
...
Examples:
    ...
    ./run.sh -t 1 \\
        ...
        -r 3 \\
        -s 100 \\
        -cc "postgresql.replication.postgresql.4c8g.general"
    
    # Scale up compute resources
    ./run.sh -t 4 \\
        -cc "postgresql.replication.postgresql.2c4g.general"
...
terraform_init_and_apply() {
    ...
    echo "Running: terraform plan -out pg_plan"
    terraform plan -out=pg_plan -var-file=terraform.tfvars
    
    echo ""
    echo "Running: terraform apply pg_plan"
    terraform apply pg_plan
