# PostgreSQL Terraform Test Script Guide

## 📖 Overview

The `run.sh` script provides a command-line interface for managing PostgreSQL clusters on KubeBlocks Cloud using Terraform. It simplifies cluster operations by allowing you to configure and execute Terraform commands through simple command-line parameters.
...
# Create a default PostgreSQL cluster
./run.sh -t 1
...
| Backup Method | `-bm` | `--backup-method` | - | Backup method (wal-g-archive) |
...
| Config File Name | `-cfn` | `--config-file-name` | - | Configuration file name for reconfigure (e.g., postgresql.conf) |
...
**What this does:**
- Creates a single-replica PostgreSQL cluster
- 10GB storage
...
- Uses wal-g-archive method for continuous backup and PITR
...
**What this does:**
- Updates PostgreSQL configuration parameters
...
echo "Deploying PostgreSQL cluster: $CLUSTER_NAME"
...
- [PostgreSQL Example README](README.md)
